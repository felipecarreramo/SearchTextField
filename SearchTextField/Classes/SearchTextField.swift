...
open class SearchTextField: UITextField {
    ...
    open var totalItemsToFilter: Int?
    
    open var loadMoreFilterItems: SearchTextFieldLoadMoreFilterItemsHandler?
    
    open var resultsViewMaxHeight: CGFloat = 200.0
    ...
    fileprivate func redrawSearchTableView() {
        ...
        if let tableView = tableView {
            ...
            if self.direction == .down {
                ...
                tableView.frame = CGRect(x: frame.origin.x + 2, y: (frame.origin.y + frame.size.height - positionGap), width: frame.size.width - 4, height: tableHeight > resultsViewMaxHeight ? resultsViewMaxHeight:  tableHeight)
